...
INPUT_DIR = Path("input/fegrow_result").absolute()
ORIGINAL_RECPT = Path("input/rec.pdb").absolute()
RESULTS_CSV = INPUT_DIR / "mol_scores_sorted.csv" 
OUTPUT_DIR = Path("abfe").absolute()
...
def pdb_to_rst7_and_prm7(pdb_path: Path, rst7_path: Path, prm7_path: Path) -> None:
    ...
    script_path = Path("tleap_script.in")
    tleap_script = f"""
...
def write_inputs_mol_and_receptor(ligand: str, receptor: str, parent_dir: Path, lig_prm7_path: Path) -> None:
    ...
    sdf_to_rst7(INPUT_DIR / ligand, input_files_dir / "ligand.rst7")
    protein_prm7_path = input_files_dir / "receptor.prm7"
    protein_rst7_path = input_files_dir / "receptor.rst7"

    # Create the merged PDB file
    merged_pdb_path = INPUT_DIR / f"{receptor_no_suffix}_pocket_only.pdb"
    merge_pdb_with_pocket(str(ORIGINAL_RECPT), str(INPUT_DIR / receptor), str(merged_pdb_path))
...
def write_inputs(mol_id: str, df: pd.DataFrame, out_dir: Path) -> None:
    ...
    # save
    ligand_path = INPUT_DIR / mol_df.iloc[0]["ligand"]
