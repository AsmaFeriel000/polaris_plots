...
def calculate_rmsd_pocket(ref_positions, rec_positions):
    """Pocket RMSD: raw coordinate RMSD (ordering stable)."""
    ref_mol = to_rdkit_mol(ref_positions)
    rec_mol = to_rdkit_mol(rec_positions)
    return AllChem.CalcRMS(ref_mol, rec_mol)


def calculate_rmsd_ligand(ref_positions, rec_positions):
    """
    Ligand RMSD: uses RDKit atom matching + alignment.
    This is REQUIRED for correct ligand RMSD.
    """
    ref_mol = to_rdkit_mol(ref_positions)
    rec_mol = to_rdkit_mol(rec_positions)
    return rdMolAlign.GetBestRMS(rec_mol, ref_mol)
...
for rec_file in sorted(glob.glob(os.path.join(query_dir, "*.pdb"))):
    ...
    # Compute RMSDs
    pocket_rmsd = calculate_rmsd_pocket(ref_pocket_pos, rec_pocket_pos)
    lig_rmsd = calculate_rmsd_ligand(ref_lig_pos, rec_lig_pos)
