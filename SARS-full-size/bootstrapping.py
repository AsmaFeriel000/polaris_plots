...
def run_all():
    ...
    test_mols = [m for m in Chem.SDMolSupplier("test_sars.sdf") if m is not None]
    ...
    plt.xlabel("Top N")
    plt.ylabel("% RMSD < 2 Å")
    plt.title("Top-N Pose Accuracy for SARS-CoV-2 Mpro")
    plt.legend()
    plt.grid(True)
    plt.xticks(np.arange(1, len(N)+1, 2)) # x axis ticks
    plt.tight_layout()
