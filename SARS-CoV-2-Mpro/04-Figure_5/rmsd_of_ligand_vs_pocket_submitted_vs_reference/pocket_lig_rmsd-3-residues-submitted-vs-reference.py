...
"""Compute ligand RMSD and pocket RMSD between submitted SARS-CoV Mpro complexes
...
Outputs results to "complex_rmsd_results-submitted-vs-ref-3-residues.csv".
"""

# ======================
# USER SETTINGS
# ======================
docked_dir = Path("../build-complexes-of-prospective-submission/output")
reference_dir = Path("../../../released-receptor-structures/group1_SARS-CoV-2_Mpro/sars_files")
...
out_csv = output_dir / "complex_rmsd_results-submitted-vs-ref-3-residues.csv"
# ======================
...
def get_molid(filename):
    m = re.search(r"complex-SARS-mol(\d+)\.pdb$", filename.name)
...
dock_files = sorted(docked_dir.glob("complex-SARS-mol*.pdb"))
...
for dock_file in dock_files:
    ...
    ref_file = reference_dir / f"sars_{molid}.pdb"
...
# complex-SARS-mol<x>.pdb
# -------------------------
def extract_molid(filename):
    m = re.search(r"complex-SARS-mol(\d+)\.pdb$", filename)
