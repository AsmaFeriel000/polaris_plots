...
def parse_args():
    ...
    parser.add_argument(
        "--input_folder",
        type=str,
        default="../../02-building-poses-in-ApoDock-receptors/output",
        help="Input folder containing ligand and receptor files.",
    )
    parser.add_argument(
        "--ligand_pattern",
        type=str,
        default="../../02-building-poses-in-ApoDock-receptors/output/cs_optimised*",
        help="Glob pattern for ligand files.",
    )
    parser.add_argument(
        "--receptor_pattern",
        type=str,
        default="../../02-building-poses-in-ApoDock-receptors/output/rec_final*",
        help="Glob pattern for receptor files.",
    )
    parser.add_argument(
        "--ckpt",
        type=str,
        default="./input/checkpoints/ApoScore_time_split_0.pt",  # comes with installing ApoDock
...
def get_scores():
    ...
    ligand_files = sorted(collect_files(os.path.join(args.input_folder, "resulting_mols"), "*.sdf"))
   ...
   # Score all receptor-ligand pairs done above, now reduce by molX
    ...
    # Write results
    output_file = os.path.join(output_dir, "best_score_per_mol.txt")
    ...
    output_sdf_path = os.path.join(output_dir, "best_score_ligands.sdf")
    ...
    output_csv = os.path.join(output_dir, "mol_scores_sorted.csv")
