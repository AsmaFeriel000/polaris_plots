...
"""calculate rmsd between top scoring FEgrow-docked ligands (binding affinity scores) and their corresponding test_sars.sdf (released test molecules) to generate cdf plot
"""

def find_best_scores_per_molecule(   
    ...
    test_sdf="../../../released-receptor-structures/released_test_molecules/test_sars.sdf"
):
    ...
    for mol_index, (rec_index, best_score) in best_per_mol.items():
        ...
        if mol_index >= len(test_mols):
            skipped.append((f"mol{mol_index}", "index out of range in test_sars.sdf"))
            continue
    ...
    csv_file = "./output/gnina_scores_rmsds.csv"
    ...
    if skipped:
        os.makedirs("./output", exist_ok=True)
        skipped_file = "./output/skipped_details.csv"
